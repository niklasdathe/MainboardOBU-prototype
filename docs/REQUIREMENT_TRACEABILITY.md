...
| DISP-001/003/005/006/008/009 | abstract HMI model + SSD1306 driver, non-blocking consumer | Implemented baseline renderer |
...
1. HIL/RF throughput, queue-overflow and 4-hour endurance tests.
2. Legal/regulatory TX configuration verification for the actual test location.
3. Complete ETSI VAM ASN.1 UPER codec and Release-2 transport stack.
4. ETSI security backend with valid credentials, signing and identity-change behavior.
5. PoTi implementation and a positioning source that satisfies the workbook's full-conformance accuracy gate.
6. DCC/GN/BTP timing/latency/reliability tests and the workbook's dense-load criteria.
7. Final carrier pin assignment and electrical verification with GNSS + SD/RTC/display + CAN + C5 link connected concurrently.
